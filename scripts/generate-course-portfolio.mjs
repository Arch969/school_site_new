...
const configs = [
  ...
  { folder:'python_base', file:'19-python-projects.html', id:'python-projects', badge:'Портфолио курса Python', title:'Первые проекты <span>на Python</span>', text:'Ученики пишут полезные мини-программы, графику и творческие проекты, закрепляя основы кода на практике.' },
...
function cardHtml(config, item, mediaName, index) {
  const color = colors[index % colors.length];
  const ext = path.extname(mediaName).toLowerCase();
  const folderUrl = `media/portfolio/${config.folder}`;
  const isGif = ext === '.gif';
  ...
  const ratioClass = config.ratio === '4-3' ? ' ku-project-card__preview--ratio-4-3' : '';
...
function blockHtml(config, items) {
  const cards = items.map((entry, index) => cardHtml(config, entry.item, entry.mediaName, index)).join('\n');
...
for (const config of configs) {
  ...
  for (let i = 0; i < desc.length; i += 1) {
    ...
    if (mediaName) entries.push({ item: desc[i], mediaName });
  }
