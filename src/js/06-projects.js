(function(){
  ...
  function setupGifLazyLoad() {
    const gifImages = Array.from(section.querySelectorAll('img[data-gif-src]'));
    if (!gifImages.length || reduce) return;

    const loadGif = (img) => {
      const gifSrc = img.getAttribute('data-gif-src');
      if (!gifSrc || img.dataset.gifLoaded === 'true') return;

      img.dataset.gifLoaded = 'loading';
      const preloader = new Image();
      preloader.decoding = 'async';
      preloader.onload = () => {
        img.src = gifSrc;
        img.dataset.gifLoaded = 'true';
      };
      preloader.onerror = () => {
        img.dataset.gifLoaded = 'error';
      };
